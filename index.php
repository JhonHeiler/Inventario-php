...
if (isset($modulos[$modulo]["verificar_permisos"]) == true) {
    ...
    $sql = "SELECT 
                COUNT(*) cantidad,r.id_rol
            FROM
                persona
                    JOIN
                persona_rol ON persona.id_persona = persona_rol.id_persona
                    JOIN
                rol r ON persona_rol.id_rol = r.id_rol
                    JOIN
                permiso_rol ON r.id_rol = permiso_rol.id_rol
            WHERE
                persona.num_docu = '$usuario'
                    AND permiso_rol.modulo1 = '$modulo'
                    AND permiso_rol.accion1 = '$accion'";
