   ...
   <div id="contenedor-listado" class="content mt-3" style="/*width:800px;*/ margin:auto">
                ...
                <form id="formulario" method="post" action="guardar.php">
                    <input type="hidden" name="id_marca" id="id_marca" />
                     ...
                     <div class="form-group row">
                        <label for="nombre" class="col-sm-3 col-form-label">Nombre </label>
                        <div class="col-sm-9">
                            <input type="text" class="form-control" id="nombre" name="nombre" placeholder="Nombre de la marca" >
                        </div>
                    </div> 
    ...
    <script type="text/javascript">
        ...
        $("#btn-guardar").click(function() {
            ...
            $.post("?modulo=marca&accion=agregar", parametros, function(respuesta) {
        ...
        $("#btn-modificar").click(function() {
            if (confirm("¿Desea modificar el registro?")) {
                var parametros = $("#formulario").serialize();
                $.post("?modulo=marca&accion=modificar", parametros, function(respuesta) {
                    //jQuery.parseJSON convertir la respuesta en JSON en un arreglo asociativo
                    var r = jQuery.parseJSON(respuesta);
                    alert(r.mensaje);
                    if (r.error == false) {
        ...
        function eliminar(id_marca) {
            if (confirm("¿Realmente desea eliminar el registro?")) {
                ...
                $.post("?modulo=marca&accion=eliminar", parametros, function(respuesta) {
                    var r = jQuery.parseJSON(respuesta);
                    alert(r.mensaje);
                    if (r.error == false) {
                        cargar_listado();
       ...
       function modificar(id_marca) {
            ...
            var parametros = "id_marca=" + id_marca;
            $.get("?modulo=marca&accion=cargar-datos", parametros, function(respuesta) {
                $("#formulario")[0].reset(); //limpiar formulario
                $("#contenedor-listado").hide();
                $("#contenedor-formulario").show();
        ...
        function cargar_listado() {
            ...
            $("#listado").load("?modulo=marca&accion=listar", parametros);
        }
