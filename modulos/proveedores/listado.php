...
$sql_cantidad = "SELECT COUNT(*) AS cantidad FROM proveedores";
...
<table class="table table-striped" >
    ...
    <tbody>
        ...
        if (isset($_GET['num_docu']) == true &&  $_GET['num_docu']  != "") {
            $filtros .= " AND pr.num_docu = '$_GET[num_docu]' ";
        }
        if (isset($_GET['nombre']) == true &&  $_GET['nombre']  != "") {
            $nombre = $_GET['nombre'];
            $nombre = str_replace(" ", "%", $nombre);
            $filtros .= " AND CONCAT_WS(' ',pr.nombre) LIKE '%$nombre%' ";
        }

        if (isset($_GET['telefono']) == true &&  $_GET['telefono']  != "") {
            $filtros .= " AND pr.telefono = '$_GET[telefono]' ";
        }

        if (isset($_GET['tipo_docu']) == true && $_GET['tipo_docu'] != "") {
            $filtros .= " AND pr.tipo_docu = '$_GET[tipo_docu]' "; 
        }
        ...
        while ($row = mysqli_fetch_assoc($rmu)) {
            echo "<tr>";
            echo "<td>$num</td>";
            echo "<td>$row[num_docu]</td>";
            echo "<td>$row[nombre]</td>";
            echo "<td>$row[telefono]</td>";
            echo "<td>$row[direccion]</td>";
             echo "<td class='acciones'>
                   <a href='javascript:;' class='mostrar'  onclick='mostrar(\"$row[id_provedor]\")'>
                        <i class='fas fa-eye'></i>
                    </a>
                    <a href='javascript:;' class='eliminar' onclick='eliminar(\"$row[id_provedor]\")'>
                        <i class='fas fa-trash'></i>
                    </a>
                    <a href='javascript:;' class='modificar' onclick='modificar(\"$row[id_provedor]\")'>
