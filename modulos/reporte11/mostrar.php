...
$sql = "SELECT p.id_producto,
                ...
                proveedores.nombre proveedor,                
                    SUM(devolucion.cantidad) total
                
                FROM producto p
                join marca on p.id_marca=marca.id_marca
                join proveedores on p.id_provedor=proveedores.id_provedor
                join devolucion on p.id_producto=devolucion.id_producto
                join persona on devolucion.responsable=persona.id_persona
...
<table style="font-family:'Times New Roman', Times, serif; font-size:8pt; 
    ...
    <thead>
        <tr style="font-weight:bold; text-align:left; background-color:#e1e1e1">
            <tr style=" text-align: center; background-color:#e1e1e1">Lista de devoluciones por marca</tr>
            ...
            <th style="width:40mm">Tol. devoluciones por marca</th>
        </tr>
    </thead>
...
<?php
        ...
        if ($formato == "html") {
            ...
            header("Content-Disposition: attachment; filename=estadistico por marca devolucion.doc;");
            echo $html;
 
 
        } else if ($formato == "xls") {
            header("Content-Type: application/download");
            header("Content-Disposition: attachment; filename=estadistico por marca devolucion.xls;");
            echo $html;
        } else if($formato=="pdf") {
            ...
            <b>Estadístico por marca devolucion</b>
            <br/> 
            </div>";
            ...
            $pdf->Output('estadistico por marca devolucion.pdf', 'I');
